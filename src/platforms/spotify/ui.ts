/**
 * Spotify Web Player UI injector.
 *
 * Injects the STO action button next to Spotify's "more" button
 * (`data-testid="more-button"`) and manages the dropdown purchase-links menu.
 *
...
const BTN_ID = "sto-sp-action-btn";
const MENU_ID = "sto-sp-dropdown-menu";

/**
 * SVG icon paths for each store.
 * Slightly different selection from YTM — uses thinner, Spotify-like icons.
 */
...
export class SpotifyUIInjector implements UIInjector {
  ...
  async injectButton(meta: MusicMetadata): Promise<void> {
    ...
    const moreBtn = await waitForElement('[data-testid="more-button"]');
    if (!moreBtn) return;
    ...
    // Insert after the more-button, inside the action bar
    moreBtn.after(btn);
  }
  ...
  private toggleMenu(anchor: HTMLElement): void {
    ...
    if (availableLinks.length === 0) {
      ...
      empty.className = "sto-sp-menu__empty";
...
/** Shopping bag icon — same concept, styled for Spotify. */
function createButtonIcon(): SVGSVGElement {
  ...
  svg.classList.add("sto-sp-btn__icon");
...
function createMenuItem(link: StoreLink): HTMLAnchorElement {
  ...
  if (svgPath) {
    ...
    svg.setAttribute("fill", "currentColor");
    svg.classList.add("sto-sp-menu__item-icon");
    const path = document.createElementNS(ns, "path");

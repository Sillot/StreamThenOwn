/**
 * Amazon Music Web Player UI injector.
 *
 * Injects the STO action button into Amazon Music's album detail header
 * icon row (`<div slot="icons">`) and manages the dropdown purchase-links
 * menu.
 *
 * Amazon Music uses custom web components (`<music-button>`) with shadow
 * DOM for native buttons. Our button is a standard `<button>` styled to
 * match the icon-only buttons (glass variant: white at 0.6 opacity,
 * teal #25d1da on hover).
 *
...
export class AmazonMusicUIInjector implements UIInjector {
  ...
  async injectButton(meta: MusicMetadata): Promise<void> {
    ...
    // Amazon Music groups action icons inside <div slot="icons">
    const iconsRow = await waitForElement('div[slot="icons"]');
    if (!iconsRow) return;
  ...
  private toggleMenu(anchor: HTMLElement): void {
    ...
    if (existing) {
      existing.remove();
      anchor.classList.remove("sto-am-btn--open");
      return;
    }
    ...
    anchor.classList.add("sto-am-btn--open");
